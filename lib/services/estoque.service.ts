import { estoqueRepository } from '@/lib/repositories/estoque.repository'
...
export class EstoqueService {
  ...
  async atualizar(userId: string, id: number, dados: any) {
    await this.obter(userId, id) // Verifica se existe
  ...
  async adicionarQuantidade(userId: string, id: number, quantidade: number, motivo: string) {
    const item = await this.obter(userId, id)
  ...
  async removerQuantidade(userId: string, id: number, quantidade: number, motivo: string) {
    const item = await this.obter(userId, id)
  ...
  async obterEstoqueBaixo(userId: string) {
  ...
  async obterResumo(userId: string) {
    const items = await this.listar(userId)
    const movimentacoes = await this.obterMovimentacoes(userId)
    const itensLagos = await this.obterEstoqueBaixo(userId)
