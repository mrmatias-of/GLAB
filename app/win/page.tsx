...
export default function WinToolPage() {
  return <main className="min-h-screen overflow-hidden bg-[#06080d] text-white">
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-14 lg:grid-cols-[.95fr_1.05fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-7 max-w-3xl text-5xl font-black leading-[.9] tracking-[-.07em] md:text-7xl">Uma central profissional para dominar o Windows.</h1>
            <p className="mt-7 max-w-xl text-lg leading-8 text-slate-300">Instalação, manutenção, diagnóstico e ajustes em uma única experiência. Menos improviso na bancada. Mais velocidade, controle e confiança em cada máquina.</p>
            <div className="mt-9 flex flex-wrap gap-3"><a href="https://github.com/mrmatias-of/GLWinTool" target="_blank" rel="noreferrer" className="inline-flex items-center gap-3 rounded-xl bg-cyan-300 px-6 py-4 text-sm font-black text-[#061014] transition hover:-translate-y-1 hover:bg-cyan-200"><Download size={17}/>Baixar o GL WinTool <ArrowRight size={16}/></a><a href="https://github.com/mrmatias-of/GLWinTool" target="_blank" rel="noreferrer" className="inline-flex items-center gap-3 rounded-xl border border-white/15 bg-white/[.04] px-6 py-4 text-sm font-black text-white hover:border-cyan-300/35"><Github size={17}/>Ver código</a></div>
    ...
    <section className="mx-auto max-w-7xl px-5 py-20 md:px-8 md:py-28"><div className="max-w-2xl"><p className="text-[10px] font-black uppercase tracking-[.22em] text-cyan-300">Uma ferramenta, seis frentes</p><h2 className="mt-4 text-4xl font-black tracking-[-.05em] md:text-6xl">Uma camada de operação para cada rotina.</h2></div><div className="mt-10 grid gap-4 lg:grid-cols-3">{features.map(({icon:Icon,label,title,text})=><article key={label} className="rounded-3xl border border-white/10 bg-white/[.03] p-7 transition hover:-translate-y-1 hover:border-cyan-300/30"><div className="flex items-center justify-between"><Icon size={23} className="text-cyan-300"/><span className="text-[10px] font-black uppercase tracking-[.2em] text-slate-500">{label}</span></div><h3 className="mt-8 text-2xl font-black">{title}</h3><p className="mt-3 text-sm leading-7 text-slate-400">{text}</p></article>)}</div></section>
