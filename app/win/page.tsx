    ...
    <section className="relative border-b border-white/10 px-5 pb-20 pt-10 md:px-8 md:pb-28 md:pt-16">
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-14 lg:grid-cols-[.95fr_1.05fr] lg:items-center">
          ...
          <div className="relative"><div className="absolute -inset-6 rounded-[38px] bg-cyan-400/10 blur-3xl"/><div className="relative overflow-hidden rounded-[28px] border border-cyan-200/20 bg-[#0b111b] shadow-2xl shadow-black/50"><div className="flex items-center justify-between border-b border-white/10 px-5 py-4"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-cyan-300/15 text-cyan-200"><Zap size={17}/></div><div><p className="text-sm font-black">GL WinTool</p><p className="text-[10px] uppercase tracking-[.16em] text-slate-500">Central Windows</p></div></div><span className="rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-[10px] font-black text-emerald-200">OPERACIONAL</span></div><div className="grid gap-3 p-5 sm:grid-cols-2"><div className="rounded-2xl border border-white/10 bg-white/[.04] p-5 sm:col-span-2"><p className="text-xs font-black uppercase tracking-[.18em] text-cyan-200">Visão geral</p><p className="mt-3 text-2xl font-black">Prepare. Ajuste. Valide.</p><p className="mt-2 text-sm leading-6 text-slate-400">Uma sequência clara para cada máquina.</p></div>{["Instalar apps","Ajustar Windows","Reparar sistema","Gerenciar AppX"].map((item)=><div key={item} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/20 p-4 text-sm font-bold text-slate-200"><Check size={16} className="text-cyan-300"/>{item}</div>)}</div></div></div>
        </div>
      </div>
    </section>
