  ...
  return <main className="min-h-screen bg-[#050712] text-white">
    <header className="sticky top-0 z-30 border-b border-white/10 bg-[#050712]/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8">
        ...
        <div className="flex items-center gap-4"><span className="hidden text-right text-xs text-slate-400 sm:block"><strong className="block text-white">{user.name}</strong>{user.email}</span><Link href="/" className="rounded-xl border border-white/10 p-2.5 text-slate-300 hover:border-cyan-300/50 hover:text-cyan-200" aria-label="Voltar ao site"><LayoutDashboard size={17} /></Link></div>
      </div>
    </header>
    {children}
  </main>
