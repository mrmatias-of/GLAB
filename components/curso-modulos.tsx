      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-10">
          ...
          <div className="sm:ml-auto flex items-center gap-4 text-sm text-white/40">
            <span><strong className="text-cyan-400">{curso.modulos.length}</strong> modulos</span>
            <span><strong className="text-cyan-400">{totalTopicos}</strong> topicos</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          {curso.modulos.map((modulo, i) => (
            <div
              key={modulo.titulo}
              className="rounded-2xl border border-white/10 overflow-hidden transition-all duration-300 hover:border-cyan-500/30"
            ...
            >
              ...
              <div className="flex items-center gap-4 px-5 py-4 border-b border-white/5">
                <div className="w-10 h-10 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center shrink-0">
                  <span className="text-sm font-black text-cyan-400">{String(i + 1).padStart(2, "0")}</span>
                </div>
                ...
                {modulo.topicos.map((topico, j) => (
                  ...
                  >
                    <div className="w-6 h-6 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                      <Play size={10} className="text-cyan-400/50" />
                    </div>
