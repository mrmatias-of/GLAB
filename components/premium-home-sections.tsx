  ...
  return <>
    ...
    <section className="relative overflow-hidden px-5 py-24 md:px-8 md:py-36 xl:px-12">
      ...
      <div className="relative mx-auto max-w-[1360px]">
        ...
        <div className="mt-14 grid gap-4 lg:grid-cols-12">{areas.map((area) => <Link href={area.href} key={area.number} className={`group relative min-h-[330px] overflow-hidden rounded-[24px] border border-white/[.11] bg-[#090c17] shadow-[0_28px_80px_rgba(0,0,0,.24)] transition duration-700 hover:-translate-y-1.5 hover:border-cyan-300/35 hover:shadow-[0_36px_100px_rgba(0,0,0,.45)] ${area.span}`}>
          ...
          <div className="relative flex min-h-[330px] flex-col p-5 md:p-7">
            ...
            <div className="mt-auto max-w-2xl rounded-2xl border border-white/10 bg-black/45 p-4 backdrop-blur-sm md:p-5"><div className="mb-3 flex items-center gap-3"><span className="h-px w-9 bg-cyan-300/70"/><span className="text-[9px] font-black uppercase tracking-[.2em] text-cyan-200">6 formações</span></div><h3 className="text-2xl font-black leading-[.95] tracking-[-.045em] text-white md:text-3xl">{area.title}</h3><p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">{area.text}</p><span className="mt-4 inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[.17em] text-white">Explorar trilha <ArrowRight size={15} className="transition group-hover:translate-x-1.5"/></span></div>
          </div>
        </Link>)}</div>
      </div>
    </section>
